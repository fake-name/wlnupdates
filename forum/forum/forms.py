...
class CreateThreadForm(FlaskForm):
	name = TextField(validators=[DataRequired(message=title_error_message_missing), Length(max=80, message=title_error_message_length)])
	content = TextAreaField(validators=[DataRequired(message=body_error_message)])
	submit = SubmitField()
