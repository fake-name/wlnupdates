...
def upgrade():
	...
	op.execute("""
		UPDATE users SET primary_group_id = 1 WHERE id = 1 OR id = 2 OR id = 3;
		""")

	op.execute("""
		UPDATE users SET primary_group_id = 4 WHERE NOT (id = 1 OR id = 2 OR id = 3);
		""")

	op.execute("""UPDATE users SET lastseen = 'now' WHERE lastseen IS NULL;""")
	op.execute("""UPDATE users SET date_joined = 'now' WHERE date_joined IS NULL;""")

	op.alter_column('users', 'post_count', nullable=False)
	op.alter_column('users', 'lastseen', nullable=False)
	op.alter_column('users', 'date_joined', nullable=False)
